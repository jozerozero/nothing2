...
#SBATCH --output=/vast/users/guangyi.chen/causal_group/zijian.li/codex/all178_crossfit_20260722_v1/logs/e4_g5sc_loop3_resume181407_eval_fp32_online_16gpu_gt_step50_20260911_v1/slurm-%j.out
#SBATCH --error=/vast/users/guangyi.chen/causal_group/zijian.li/codex/all178_crossfit_20260722_v1/logs/e4_g5sc_loop3_resume181407_eval_fp32_online_16gpu_gt_step50_20260911_v1/slurm-%j.err
set -euo pipefail
...
LOG_ROOT=/vast/users/guangyi.chen/causal_group/zijian.li/codex/all178_crossfit_20260722_v1/logs/e4_g5sc_loop3_resume181407_eval_fp32_online_16gpu_gt_step50_20260911_v1/E4_G5SC_LOOP3/lineage-178786-181407/job-${SLURM_JOB_ID}
...
python3 -c 'import sys; sys.path.insert(0,"'"${STAGE}"'"); from recovery import runtime_contract; runtime_contract()'
...
srun --gpu-bind=single:1 --nodes=4 --ntasks=16 --ntasks-per-node=4 --gpus-per-task=1 --cpus-per-task=4   --kill-on-bad-exit=1 --wait=0   --output="${LOG_ROOT}/group-task-%t-${SLURM_JOB_ID}.out"   --error="${LOG_ROOT}/group-task-%t-${SLURM_JOB_ID}.err"   bash "${STAGE}/run_group_worker.sh"
for task in $(seq 0 15); do test -s "${JOB_ROOT}/task-${task}/task.complete"; done
python3 - "${OUTPUT_ROOT}" <<'PY'
import csv, math, sys, time
from pathlib import Path
root = Path(sys.argv[1])
expected = [root / f"step-{step}" / "talent_detailed.txt" for step in range(8850, 25001, 50)]
def scan():
    signatures = {}
...
PY
printf 'job_id=%s training_lineage=178786,181407 groups=4 shards=4 gpus=16 strict=324 explicit_fp32=1 complete=1
