...
test "${SLURM_JOB_NAME}" = e4g5sc4r4st1
test "${SLURM_NNODES}" -eq 1
test "${SLURM_NTASKS}" -eq 4
test "${SLURM_JOB_QOS}" = stqos
...
srun --gpu-bind=single:1 --nodes=1 --ntasks=4 --ntasks-per-node=4 --gpus-per-task=1 --cpus-per-task=4   --kill-on-bad-exit=1 --wait=0   --output="${LOG_ROOT}/group-task-%t-${SLURM_JOB_ID}.out"   --error="${LOG_ROOT}/group-task-%t-${SLURM_JOB_ID}.err"   bash "${STAGE}/run_group_worker.sh"
for task in $(seq 0 3); do test -s "${JOB_ROOT}/task-${task}/task.complete"; done
