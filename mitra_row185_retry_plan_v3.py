...
def main():
    d.configure_common()
    c = d.common
    parent, attempt = '204827', 'row185_retry3'
    a = c.read(d.ROOT / 'capacity_row185_retry3_a.json')
    b = c.read(d.ROOT / 'capacity_row185_retry3_b.json')
    ...
    script = 'pfn_mitra8_row185_retry_v3.py'
    plan.pop('plan_id')
    plan.update(created_epoch=time.time(), dataset_indices=[185], target_results=1,
        ...
        capacity_evidence={'first': str(d.ROOT / 'capacity_row185_retry3_a.json'),
                           'second': str(d.ROOT / 'capacity_row185_retry3_b.json')},
        implementation_exception='Independent attention batch-axis tiling and pointwise activation memory reuse; full K/V, native1024query chunks and RNG unchanged')
    plan['worker_sha256'][script] = c.digest(c.REPO / script)
    plan['worker_sha256']['mitra_row185_retry_plan_v3.py'] = c.digest(c.REPO / 'mitra_row185_retry_plan_v3.py')
    plan['plan_id'] = hashlib.sha256(json.dumps(plan, sort_keys=True).encode()).hexdigest()
    path = d.ROOT / 'plan_row185_retry3.json'
