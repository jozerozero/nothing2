...
Version 2 is the separately authorized FP32/no-AMP protocol. It disables an
unconditional native classification autocast context without editing the
official source. Every floating model/module boundary is audited as FP32.
Native probabilities are validated strictly, never renormalized or repaired.
"""
...
def protocol_settings(campaign, task):
    ...
    require(protocol.get("use_amp") is False and protocol.get("precision") == "fp32",
            "Version 2 requires an explicit FP32/no-AMP campaign")
    return {"n_estimators": counts[task], "norm_methods": ["none", "power"],
        ...
        "use_hierarchical": True, "batch_size": 16, "use_amp": False,
...
@contextmanager
def fp32_native_guard(torch, estimator):
    """Disable native AMP locally and audit every model-module boundary.

    The pinned ICLPredictor._predict_standard unconditionally enters
    torch.autocast("cuda"), ignoring its use_amp=False setting. A scoped
    factory adapter disables that context; all arguments except enabled are
    preserved, no model method/checkpoint/source/prediction is rewritten.
    This worker runs one estimator in one process, with no inference threads.
    """
    require(estimator.use_amp is False, "Native estimator must disable AMP")
    model = estimator.model_
    require(not model.training, "FP32 audit requires evaluation mode")
    original_autocast = torch.autocast
    previous_precision = torch.get_float32_matmul_precision()
    previous_matmul_tf32 = torch.backends.cuda.matmul.allow_tf32
    previous_cudnn_tf32 = torch.backends.cudnn.allow_tf32
    handles = []
    audit = {"requested_precision": "fp32", "native_use_amp": False,
             "autocast_requests_disabled": 0, "forward_pre_hook_calls": 0,
             "forward_hook_calls": 0, "checked_float_tensor_inputs": 0,
             "checked_float_tensor_outputs": 0, "autocast_enabled_observations": 0,
             "probabilities_renormalized": False, "native_methods_replaced": False,
             "official_source_modified": False,
             "compatibility_adapter": "scoped torch.autocast enabled=False for pinned unconditional native classification context"}

    def state_dtypes():
        parameters = list(model.parameters())
        floating = [p for p in parameters if p.is_floating_point()]
        buffers = [p for p in model.buffers() if p.is_floating_point()]
        require(floating and all(p.dtype == torch.float32 for p in floating + buffers),
                "Native parameters/buffers are not entirely FP32")
        return sorted({str(p.dtype) for p in floating}), sorted({str(p.dtype) for p in buffers})
    ...
    def check_context():
        enabled = torch.is_autocast_enabled("cuda") or torch.is_autocast_enabled("cpu")
        audit["autocast_enabled_observations"] += int(enabled)
        require(not enabled, "AMP enabled during native FP32 inference")

    def check_tensors(value, counter):
        for tensor in tensors(value):
            if tensor.is_floating_point():
                require(tensor.dtype == torch.float32, "Non-FP32 tensor at native model boundary")
                audit[counter] += 1

    def before(module, args, kwargs):
        check_context()
        if module is model:
            require(kwargs.get("use_amp") is False, "Native model forward did not disable AMP")
        check_tensors((args, kwargs), "checked_float_tensor_inputs")
        audit["forward_pre_hook_calls"] += 1

    def after(module, args, kwargs, result):
        check_context()
        check_tensors(result, "checked_float_tensor_outputs")
        audit["forward_hook_calls"] += 1

    def disabled_autocast(*args, **kwargs):
        positional = list(args)
        options = dict(kwargs)
        device_type = positional[0] if positional else options.get("device_type")
        require(device_type in ("cuda", "cpu") and len(positional) <= 4,
                "Unexpected native autocast invocation")
        if len(positional) >= 3:
            require("enabled" not in options, "Duplicated native autocast enabled argument")
            positional[2] = False
        else:
            options["enabled"] = False
        audit["autocast_requests_disabled"] += 1
        return original_autocast(*positional, **options)

    try:
        audit["parameter_dtypes"], audit["floating_buffer_dtypes"] = state_dtypes()
        torch.set_float32_matmul_precision("highest")
        torch.backends.cuda.matmul.allow_tf32 = False
        torch.backends.cudnn.allow_tf32 = False
        audit.update(float32_matmul_precision=torch.get_float32_matmul_precision(),
                     matmul_allow_tf32=torch.backends.cuda.matmul.allow_tf32,
                     cudnn_allow_tf32=torch.backends.cudnn.allow_tf32)
        require(audit["float32_matmul_precision"] == "highest"
                and not audit["matmul_allow_tf32"] and not audit["cudnn_allow_tf32"],
                "FP32 matmul precision configuration not applied")
        ...
        # Also neutralize an inherited outer autocast context for both devices.
        with original_autocast(device_type="cuda", enabled=False), original_autocast(device_type="cpu", enabled=False):
            check_context()
            yield audit
            check_context()
        require(audit["forward_hook_calls"] > 0 and audit["checked_float_tensor_inputs"] > 0
                and audit["checked_float_tensor_outputs"] > 0, "No actual FP32 forward evidence")
        require(state_dtypes() == (audit["parameter_dtypes"], audit["floating_buffer_dtypes"]),
                "Model precision changed during prediction")
    finally:
...
def validate_native_probabilities(probabilities, expected_rows, expected_classes):
    """Strict FP32 validation; no change to native values or decision rule."""
    import numpy as np
    values = np.asarray(probabilities)
    require(values.shape == (expected_rows, expected_classes)
            and expected_rows > 0 and expected_classes > 0
            and values.dtype == np.dtype("float32"),
            "Invalid full-test native probabilities: shape or non-FP32 dtype")
    require(np.isfinite(values).all() and (values >= 0).all() and (values <= 1).all(),
            "Invalid full-test native probabilities: nonfinite or outside [0,1]")
    epsilon = float(np.finfo(values.dtype).eps)
    tolerance = 2e-5
    row_sum_error = np.abs(np.sum(values, axis=1, dtype=np.float64) - 1.0)
    maximum_error = float(np.max(row_sum_error))
    require(maximum_error <= tolerance,
            f"Invalid full-test native probabilities: row-sum error {maximum_error} exceeds {tolerance}")
    return {"native_dtype": str(values.dtype), "native_dtype_epsilon": epsilon,
            "maximum_absolute_row_sum_error": maximum_error,
            "absolute_row_sum_tolerance": tolerance, "relative_tolerance": 0.0,
            "tolerance_rule": "fixed2e-5 (original atol1e-5 + rtol1e-5 against one)",
            "row_sum_accumulator_dtype": "float64",
            "probabilities_renormalized": False, "native_argmax_unchanged": True}


def predict_native(estimator, train, ys, test, task, strict_actual_count, configure, torch):
    import numpy as np
    ...
    try:
        ...
        if classes is not None:
            ...
            with fp32_native_guard(torch, estimator) as precision_audit:
                native_proba = np.asarray(estimator.predict_proba(test))
            probability_validation = validate_native_probabilities(native_proba, len(test), len(classes))
            # Keep the original worker's output cast and decision rule unchanged.
            proba = native_proba.astype(np.float32)
            ...
            with fp32_native_guard(torch, estimator) as precision_audit:
                native_prediction = np.asarray(estimator.predict(test))
            require(native_prediction.dtype == np.dtype("float32"), "Native regression output is not FP32")
            prediction = native_prediction.astype(np.float32)
    ...
    with threadpool_limits(limits=args.threads):
        ...
        with torch.inference_mode(), checkpoint_load_guard(torch, campaign["weights"]["shared"]) as load_audit:
            prediction, ensemble = predict_native(estimator, train, fit_y, test, args.task_kind,
                campaign["protocol"]["strict_actual_count"], configure, torch)
        ...
        return {"complete": True, "status": "complete", "schema_version": 1,
            ...
            "protocol_description": "official recipe except separately authorized FP32/no-AMP precision, frozen standard681 support/test splits and seed42; not paper benchmark reproduction",
            ...
            "precision": ensemble["precision_audit"],
