...
Only this process's functional SDPA and fresh-Linear GELU are wrapped;
...
BATCH_CHUNK = 8
GELU_CHUNK_ELEMENTS = 64 * 1024 * 1024
...
class FreshLinearGELU:
    """Only consume tagged, full-storage outputs of native Layer.linear1/3."""
    def __init__(self, torch, original):
        self.torch, self.original = torch, original
        self.enabled = True
        self.phase = "parity"
        self.chunk_elements = GELU_CHUNK_ELEMENTS
        self.pending = {}
        self.hooks = []
    ...
    def hook(self, origin):
        def mark(module, inputs, output):
            if not self.enabled:
                return
            require(isinstance(module, self.torch.nn.Linear)
                    and isinstance(output, self.torch.Tensor), "Only native Linear tensor outputs may be tagged")
            require(not self.torch.is_grad_enabled() and not output.requires_grad,
                    "In-place GELU is strictly no-grad inference")
            require(output.is_contiguous() and output.storage_offset() == 0
                    and output.untyped_storage().nbytes() == output.numel() * output.element_size(),
                    "Native Linear output does not exclusively span its fresh contiguous storage")
            pointer = output.untyped_storage().data_ptr()
            require(all(not isinstance(value, self.torch.Tensor)
                        or value.untyped_storage().data_ptr() != pointer for value in inputs),
                    "Linear output aliases its input")
            token = id(output)
            def discard(reference, token=token):
                current = self.pending.get(token)
                if current is not None and current[0] is reference:
                    self.pending.pop(token, None)
            self.pending[token] = (weakref.ref(output, discard), origin, output._version, pointer)
        return mark

    def register_model(self, model, layer_type):
        layers = [module for module in model.modules() if isinstance(module, layer_type)]
        require(len(layers) > 0, "No pinned native Layer modules found")
        for layer in layers:
            for name in ("linear1", "linear3"):
                module = getattr(layer, name)
                require(id(module) not in self.registered_modules, "Native Linear hook already installed")
                require(isinstance(module, self.torch.nn.Linear), "Pinned native Layer MLP layout changed")
                self.registered_modules.add(id(module))
                self.hooks.append(module.register_forward_hook(self.hook(name)))

    def __call__(self, value, approximate="none"):
        tagged = self.pending.pop(id(value), None) if self.enabled else None
        if tagged is None:
            return self.original(value, approximate=approximate)
        reference, origin, version, pointer = tagged
        require(reference() is value and value._version == version
                and value.untyped_storage().data_ptr() == pointer,
                "Tagged Linear output identity/version changed before GELU")
        require(not self.torch.is_grad_enabled() and not value.requires_grad
                and value.is_contiguous() and approximate == "none",
                "Unexpected native GELU mode or tensor")
        count = math.ceil(value.numel() / self.chunk_elements)
        if self.phase == "evaluation":
            self.calls += 1
            self.kernel_calls += count
            shape_key = json.dumps([origin, list(value.shape), str(value.dtype)])
            if shape_key not in self.shapes:
                require(len(self.shapes) < 128, "Unexpectedly many native GELU shapes")
                self.shapes[shape_key] = {"origin": origin, "shape": list(value.shape),
                    "dtype": str(value.dtype), "chunks_per_call": count, "calls": 0}
                print(json.dumps({"row185_native_gelu_shape": self.shapes[shape_key]}), flush=True)
            self.shapes[shape_key]["calls"] += 1
        try:
            flat = value.view(-1)
            for start in range(0, flat.numel(), self.chunk_elements):
                part = flat[start:start+self.chunk_elements]
                part.copy_(self.original(part, approximate=approximate))
            return value
        except self.torch.cuda.OutOfMemoryError as exc:
            raise RuntimeError("Row185 bounded fresh-Linear GELU OOM; context fallback prohibited") from exc

    def audit(self):
        return {"gelu_fresh_linear_outputs_processed": self.calls,
                "gelu_kernel_calls": self.kernel_calls,
                "gelu_chunk_elements": GELU_CHUNK_ELEMENTS,
                "gelu_maximum_temporary_bytes_bfloat16": GELU_CHUNK_ELEMENTS * 2,
                "gelu_allowed_origins": ["native Layer.linear1", "native Layer.linear3"],
                "gelu_registered_native_linear_modules": len(self.registered_modules),
                "gelu_native_shapes": list(self.shapes.values())}
...
def gelu_parity(torch, wrapped):
    records = []
    old_chunk = wrapped.chunk_elements
    try:
        wrapped.chunk_elements = 127
        with torch.random.fork_rng(devices=[0]), torch.no_grad():
            for device in ("cpu", "cuda:0"):
                for dtype in (torch.float32, torch.bfloat16):
                    layer = torch.nn.Linear(17, 33, device=device, dtype=dtype).eval()
                    inputs = torch.randn((3, 11, 17), device=device, dtype=dtype)
                    handle = layer.register_forward_hook(wrapped.hook("parity_linear"))
                    try:
                        fresh = layer(inputs)
                        expected = wrapped.original(fresh)
                        actual = wrapped(fresh)
                        require(actual is fresh, "GELU parity failed to exercise in-place path")
                        records.append(close_record(torch, expected, actual,
                            kind="fresh_linear_gelu_multichunk", device=device, dtype=dtype))
                    finally:
                        handle.remove()
    finally:
        wrapped.chunk_elements = old_chunk
    return records
...
def full_model_parity(torch, functional, wrapped, gelu, model):
...
def main(argv=None):
    ...
    def load_runtime(stage, weights_manifest):
        ...
        gelu = FreshLinearGELU(torch, functional.gelu)
        state["gelu"] = gelu
        ...
        state["parity"].extend(gelu_parity(torch, gelu))
        functional.scaled_dot_product_attention = wrapped
        functional.gelu = gelu
        ...
        class RetryTrainer(guarded):
            def __init__(self, cfg, model, *a, **kw):
                ...
                gelu.register_model(self.model, native.Layer)
                ...
                if not state["full_model_parity_done"]:
                    state["parity"].extend(full_model_parity(torch, functional, wrapped, gelu, self.model))
                    state["full_model_parity_done"] = True
                    wrapped.phase = "evaluation"
                    gelu.phase = "evaluation"
    ...
    def publish(path, result):
        ...
        require(state["gelu"].calls > 0 and state["gelu"].chunk_elements == GELU_CHUNK_ELEMENTS,
                "Fresh Linear GELU implementation/audit was not exercised")
        result["base_worker_source_sha256"] = result["worker_source_sha256"]
        result["worker_source_sha256"] = base.sha256_file(__file__)
        result["implementation_audit"] = {"retry": "row185_sdpa_batch8_fresh_linear_gelu_v3",
            ...
            **state["wrapped"].audit(), **state["gelu"].audit()}
    ...
    try:
        ...
        if "gelu" in state:
            state["functional"].gelu = state["gelu"].original
            state["gelu"].close()
        if "si" in state:
