...
class TabICLClassifier(ClassifierMixin, TabICLBaseEstimator):
    ...
    use_pseudo_ssmax_thinking : bool, default=False
        If True, run a second prediction pass: the first pass pseudo-labels a
        prefix of the query rows, appends them to the in-context training set,
        and predicts the remaining query rows again. The default is disabled and
        preserves the original single-pass inference path.

    pseudo_thinking_max_passes : int, default=2
        Maximum number of pseudo-label thinking passes, including the first
        ordinary pass. Set to 3--5 to enable iterative/adaptive inference.

    pseudo_thinking_consistency_threshold : float, default=0.995
        Stop iterative pseudo-thinking early when the label consistency between
        consecutive passes on the still-query rows reaches this threshold. Set
        to 0 or below to always run up to ``pseudo_thinking_max_passes``.
    ...
    def __init__(
        ...
        support_prior_match_alpha: float = 0.0,
        average_logits: bool = True,
        support_many_classes: bool = True,
        batch_size: Optional[int] = 8,
        kv_cache: bool | str = False,
        use_pseudo_ssmax_thinking: bool = False,
        pseudo_thinking_fraction: float = 0.25,
        pseudo_thinking_conf_threshold: float = 0.0,
        pseudo_thinking_max_passes: int = 2,
        pseudo_thinking_consistency_threshold: float = 0.995,
        model_path: Optional[str | Path] = None,
        ...
        self.support_prior_match_alpha = support_prior_match_alpha
        self.average_logits = average_logits
        self.support_many_classes = support_many_classes
        self.batch_size = batch_size
        self.kv_cache = kv_cache
        self.use_pseudo_ssmax_thinking = use_pseudo_ssmax_thinking
        self.pseudo_thinking_fraction = pseudo_thinking_fraction
        self.pseudo_thinking_conf_threshold = pseudo_thinking_conf_threshold
        self.pseudo_thinking_max_passes = pseudo_thinking_max_passes
        self.pseudo_thinking_consistency_threshold = pseudo_thinking_consistency_threshold
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> TabICLClassifier:
        ...
        self.X_train_encoded_ = X
        self.y_train_encoded_ = y

        # Fit ensemble generator to create multiple dataset views
        self.ensemble_generator_ = self._make_ensemble_generator()
    ...
    def _make_ensemble_generator(self) -> EnsembleGenerator:
    ...
    def _aggregate_ensemble_outputs(self, outputs: np.ndarray, class_shuffles: List[List[int]]) -> np.ndarray:
    ...
    def _predict_proba_with_augmented_pseudo_context(
        self, X: np.ndarray, pseudo_proba: np.ndarray, pseudo_count: int
    ) -> np.ndarray:
        pseudo_y = pseudo_proba[:pseudo_count].argmax(axis=1).astype(self.y_train_encoded_.dtype, copy=False)
        X_aug = np.concatenate([self.X_train_encoded_, X[:pseudo_count]], axis=0)
        y_aug = np.concatenate([self.y_train_encoded_, pseudo_y], axis=0)
        X_remaining = X[pseudo_count:]

        pseudo_generator = self._make_ensemble_generator()
        pseudo_generator.fit(X_aug, y_aug)

        outputs = []
        for norm_method, (Xs, ys) in pseudo_generator.transform(X_remaining, mode="both").items():
            feature_shuffles = pseudo_generator.feature_shuffles_[norm_method]
            outputs.append(self._batch_forward(Xs, ys, feature_shuffles))
        outputs = np.concatenate(outputs, axis=0)

        class_shuffles = []
        for shuffles in pseudo_generator.class_shuffles_.values():
            class_shuffles.extend(shuffles)
        return self._aggregate_ensemble_outputs(outputs, class_shuffles)

    def _predict_proba_with_pseudo_thinking(self, X: np.ndarray, first_proba: np.ndarray) -> np.ndarray:
        n_test = X.shape[0]
        fraction = float(self.pseudo_thinking_fraction)
        max_passes = max(1, int(self.pseudo_thinking_max_passes))
        if n_test <= 1 or fraction <= 0.0 or max_passes <= 1:
            return first_proba

        if getattr(self, "X_train_encoded_", None) is None or getattr(self, "y_train_encoded_", None) is None:
            raise RuntimeError(
                "Pseudo-label thinking inference requires the fitted training data. "
                "Re-fit the estimator or save/load it with training data."
            )

        step_count = min(n_test - 1, max(1, int(round(n_test * min(fraction, 0.95)))))
        final_proba = first_proba.copy()
        conf_threshold = float(self.pseudo_thinking_conf_threshold)
        consistency_threshold = float(self.pseudo_thinking_consistency_threshold)

        for pass_idx in range(2, max_passes + 1):
            pseudo_count = min(n_test - 1, step_count * (pass_idx - 1))
            pseudo_conf = final_proba[:pseudo_count].max(axis=1)
            if conf_threshold > 0.0 and float(np.mean(pseudo_conf)) < conf_threshold:
                break

            previous_remaining = final_proba[pseudo_count:].argmax(axis=1)
            next_remaining_proba = self._predict_proba_with_augmented_pseudo_context(
                X, final_proba, pseudo_count
            )
            next_remaining = next_remaining_proba.argmax(axis=1)

            candidate = final_proba.copy()
            candidate[pseudo_count:] = next_remaining_proba
            final_proba = candidate

            if consistency_threshold > 0.0 and next_remaining.size > 0:
                consistency = float(np.mean(next_remaining == previous_remaining))
                if consistency >= consistency_threshold:
                    break

        return final_proba

    def _apply_support_prior_match(self, proba: np.ndarray) -> np.ndarray:
        alpha = float(getattr(self, "support_prior_match_alpha", 0.0) or 0.0)
        if alpha <= 0.0:
            return proba
        if not hasattr(self, "y_train_encoded_"):
            warnings.warn(
                "support_prior_match_alpha is ignored because the estimator has no fitted support labels.",
                UserWarning,
            )
            return proba

        num_classes = int(proba.shape[1])
        y_support = np.asarray(self.y_train_encoded_).reshape(-1)
        valid = (y_support >= 0) & (y_support < num_classes)
        if not np.any(valid):
            return proba

        eps = 1e-6
        support_counts = np.bincount(y_support[valid].astype(np.int64), minlength=num_classes).astype(np.float64)
        support_prior = (support_counts + eps) / (support_counts.sum() + eps * max(num_classes, 1))
        pred_prior = np.asarray(proba, dtype=np.float64).mean(axis=0)
        pred_prior = np.clip(pred_prior, eps, None)
        pred_prior = pred_prior / pred_prior.sum()
        scale = np.power(support_prior / pred_prior, alpha)
        calibrated = np.asarray(proba, dtype=np.float64) * scale[None, :]
        calibrated = calibrated / np.clip(calibrated.sum(axis=1, keepdims=True), eps, None)
        return calibrated.astype(proba.dtype, copy=False)
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            assert n_estimators == len(class_shuffles)
            proba = self._aggregate_ensemble_outputs(outputs, class_shuffles)

            if getattr(self, "use_pseudo_ssmax_thinking", False) and feature_mask is None:
                if (
                    getattr(self, "X_train_encoded_", None) is not None
                    and getattr(self, "y_train_encoded_", None) is not None
                ):
                    proba = self._predict_proba_with_pseudo_thinking(X, proba)
                else:
                    warnings.warn(
                        "Pseudo-label thinking inference is disabled because this estimator was saved without "
                        "training data.",
                        UserWarning,
                    )

            proba = self._apply_support_prior_match(proba)
            return proba
        finally:
