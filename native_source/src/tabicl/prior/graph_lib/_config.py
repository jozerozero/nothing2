...
class PriorConfig:
    ...
    regression_target_prior: Literal['default', 'rw_sample50'] = 'default'
    regression_target_profile: Optional[str] = None
    regression_target_mix_probability: float = 0.5
    ...
    def from_args(args) -> "PriorConfig":
        return PriorConfig(add_gaussian_noise=args.graph_noise,
                           ...
                           regression_target_prior=args.regression_target_prior,
                           regression_target_profile=args.regression_target_profile,
                           regression_target_mix_probability=args.regression_target_mix_probability)
    ...
    def add_args_to_parser(parser: argparse.ArgumentParser):
        ...
        parser.add_argument(
            "--regression_target_prior",
            choices=("default", "rw_sample50"),
            default="default",
            help="Regression target prior: exact upstream default or train-only anonymous RW-Sample50.",
        )
        parser.add_argument(
            "--regression_target_profile",
            default=None,
            help="Anonymous train-only target quantile profile required by rw_sample50.",
        )
        parser.add_argument(
            "--regression_target_mix_probability",
            type=float,
            default=0.5,
            help="Probability of empirical target-shape transfer in rw_sample50 mode.",
        )
        parser.add_argument(
