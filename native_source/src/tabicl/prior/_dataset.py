...
MAX_NONFINITE_RESAMPLE_ATTEMPTS = 64
...
class GraphPrior(Prior):
    ...
    def generate_dataset(self, params: Dict[str, Any]) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        while True:
            try:
                X, y = GraphSCM(**params)()
            except NonFinitePriorError as exc:
                nonfinite_attempts += 1
                if nonfinite_attempts >= MAX_NONFINITE_RESAMPLE_ATTEMPTS:
                    raise RuntimeError(
                        "unable to generate a finite synthetic regression task "
                        f"after {MAX_NONFINITE_RESAMPLE_ATTEMPTS} attempts"
                    ) from exc
                continue
