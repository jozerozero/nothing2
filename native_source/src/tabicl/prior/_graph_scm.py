...
from ._regression_target_prior import process_rw_sample_regression_target
from ._support_only_preprocessing import (
    NonFinitePriorError,
    support_only_outlier_remove_and_standardize,
)
    ...
    def __call__(self) -> None:
        ...
        if self.regression:
            if self.train_size is None:
                raise ValueError("regression feature preprocessing requires train_size")
            X = support_only_outlier_remove_and_standardize(
                X,
                train_size=int(self.train_size),
                threshold=4.0,
            )
        else:
            # Preserve the upstream classification path exactly.
        ...
        if self.regression:
            ...
            if config.regression_target_prior == "default":
                # Preserve the upstream implementation exactly for the control.
                y = outlier_removing(y, threshold=4)
                y = standard_scaling(y)
            elif config.regression_target_prior == "rw_sample50":
                if self.train_size is None:
                    raise ValueError("rw_sample50 regression prior requires train_size")
                y = process_rw_sample_regression_target(
                    y,
                    train_size=int(self.train_size),
                    profile_path=config.regression_target_profile or "",
                    mix_probability=config.regression_target_mix_probability,
                )
            else:
                raise ValueError(f"unknown regression_target_prior={config.regression_target_prior!r}")
            y = y.view(-1)  # (seq_len,)
        else:
        ...
        if not torch.isfinite(X).all() or not torch.isfinite(y).all():
            if self.regression:
                raise NonFinitePriorError(
                    "graph regression prior produced non-finite outputs"
                )
            # Preserve the upstream classification sentinel behavior exactly.
