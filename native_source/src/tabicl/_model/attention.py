...
_use_flash_attn3 = os.getenv("TABICL_USE_FLASH_ATTENTION", "True").lower() not in {"0", "false", "no", "off"}
...
def sdpa_with_flattened_batch(
    ...
    # FlashAttention varlen kernels use this fast path only without masks/dropout.
    if (HAS_FLASH_ATTN3 or HAS_FLASH_ATTN2) and _use_flash_attn3 and q.is_cuda and attn_mask is None and dropout_p == 0.0:
        ...
        flash_attn_varlen = flash_attn3 if HAS_FLASH_ATTN3 else flash_attn2
        out = flash_attn_varlen(q_fa, k_fa, v_fa, cu_seqlens_q, cu_seqlens_k, seqlen_q, seqlen_k)
...
def multi_head_attention_forward(
    ...
    ssmax_layer: Optional[nn.Module] = None,
    qk_pds_log_scale: Optional[Tensor] = None,
    ...
    *batch_shape, tgt_len, embed_dim = query.shape
    ...
    if qk_pds_log_scale is not None:
        expected = (num_heads, head_dim)
        if tuple(qk_pds_log_scale.shape) != expected:
            raise ValueError(
                f"qk_pds_log_scale must have shape {expected}, got {tuple(qk_pds_log_scale.shape)}"
            )
        # G3: normalize Q/K independently inside every head, then apply one
        # positive learned scale per query dimension.  The FP32 reduction is
        # deliberate for AMP stability; the projections retain their dtype.
        q_dtype, k_dtype = q.dtype, k.dtype
        q_fp32, k_fp32 = q.float(), k.float()
        q_fp32 = q_fp32 * torch.rsqrt(q_fp32.square().mean(dim=-1, keepdim=True) + 1e-6)
        k_fp32 = k_fp32 * torch.rsqrt(k_fp32.square().mean(dim=-1, keepdim=True) + 1e-6)
        scale = qk_pds_log_scale.float().clamp(-4.0, 4.0).exp()
        scale = scale.view(*([1] * len(batch_shape)), num_heads, 1, head_dim)
        q = (q_fp32 * scale).to(q_dtype)
        k = k_fp32.to(k_dtype)
    ...
    if sdpa_gate is not None:
        expected_prefix = (*batch_shape, num_heads, tgt_len)
        if sdpa_gate.shape[:-1] != expected_prefix or sdpa_gate.shape[-1] not in (1, head_dim):
            raise ValueError(
                "sdpa_gate must have shape "
                f"{expected_prefix + (1,)} or {expected_prefix + (head_dim,)}, got {tuple(sdpa_gate.shape)}"
            )
        attn_output = attn_output * sdpa_gate.to(device=attn_output.device, dtype=attn_output.dtype)
