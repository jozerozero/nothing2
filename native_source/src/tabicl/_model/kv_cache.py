...
class TabICLCache:
    ...
    schema_context : Optional[Tensor]
        Cached support-schema embedding for schema-conditioned adapters/FiLM.

    function_context : Optional[Tensor]
        Cached support-side function-token memory for query-row updates.

    attention_gate_context : Optional[Tensor]
        Cached support-only schema and group-RFF context for attention gates.
    ...
    attention_gate_context: Optional[Tensor] = None
    ...
    def cache_size_mb(self) -> int:
        ...
        # Count memory from schema context
        if self.schema_context is not None:
            total += self.schema_context.numel() * self.schema_context.element_size()
        # Count memory from function-token context
        if self.function_context is not None:
            total += self.function_context.numel() * self.function_context.element_size()
        if self.attention_gate_context is not None:
            total += self.attention_gate_context.numel() * self.attention_gate_context.element_size()
    ...
    def slice_batch(self, start: int, end: int) -> TabICLCache:
        ...
        return TabICLCache(
            ...
            schema_context=self.schema_context[indices] if self.schema_context is not None else None,
            function_context=self.function_context[indices] if self.function_context is not None else None,
            attention_gate_context=(
                self.attention_gate_context[indices] if self.attention_gate_context is not None else None
            ),
    ...
    def to(self, device, dtype=None) -> TabICLCache:
        ...
        return TabICLCache(
            ...
            schema_context=(
                self.schema_context.to(device=device, dtype=dtype) if self.schema_context is not None else None
            ),
            function_context=(
                self.function_context.to(device=device, dtype=dtype) if self.function_context is not None else None
            ),
            attention_gate_context=(
                self.attention_gate_context.to(device=device, dtype=dtype)
                if self.attention_gate_context is not None
                else None
            ),
    ...
    def concat(caches: List[TabICLCache], dim: int = 0) -> TabICLCache:
        ...
        schema_contexts = [c.schema_context for c in caches if c.schema_context is not None]
        function_contexts = [c.function_context for c in caches if c.function_context is not None]
        attention_gate_contexts = [c.attention_gate_context for c in caches if c.attention_gate_context is not None]
        ...
        return TabICLCache(
            ...
            schema_context=torch.cat(schema_contexts, dim=dim) if schema_contexts else None,
            function_context=torch.cat(function_contexts, dim=dim) if function_contexts else None,
            attention_gate_context=(
                torch.cat(attention_gate_contexts, dim=dim) if attention_gate_contexts else None
            ),
