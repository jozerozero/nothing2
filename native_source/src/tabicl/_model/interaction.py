...
class RowInteraction(nn.Module):
    ...
    encoder, optionally with rotary positional encoding. It prepends learnable class tokens to the
    ...
    use_rope : bool, default=True
        Whether to use rotary positional encoding in the row interaction transformer.
    ...
    def __init__(
        ...
        output_num_cls: Optional[int] = None,
        reference_num_cls: Optional[int] = None,
        extra_cls_init_seed: int = 2026082704,
    ) -> None:
        super().__init__()
        self.embed_dim = embed_dim
        self.num_blocks = num_blocks
        self.num_cls = num_cls
        self.output_num_cls = int(output_num_cls if output_num_cls is not None else num_cls)
        self.reference_num_cls = int(
            reference_num_cls if reference_num_cls is not None else self.output_num_cls
        )
        if self.output_num_cls <= 0 or self.num_cls % self.output_num_cls != 0:
            raise ValueError("num_cls must be divisible by output_num_cls")
        if not 0 < self.reference_num_cls <= self.num_cls:
            raise ValueError("reference_num_cls must be within [1, num_cls]")
        ...
        reference_cls_count = self.reference_num_cls
        reference_cls = torch.empty(reference_cls_count, embed_dim)
        nn.init.trunc_normal_(reference_cls, std=0.02)
        if self.num_cls > reference_cls_count:
            # Preserve the reference four-token RNG stream and initialize only
            # the four G4-only tokens in an isolated namespace.
            with torch.random.fork_rng(devices=[]):
                torch.manual_seed(int(extra_cls_init_seed))
                extra_cls = torch.empty(self.num_cls - reference_cls_count, embed_dim)
                nn.init.trunc_normal_(extra_cls, std=0.02)
            reference_cls = torch.cat((reference_cls, extra_cls), dim=0)
        self.cls_tokens = nn.Parameter(reference_cls)
        ...
        self.inference_mgr = InferenceManager(
            enc_name="tf_row", out_dim=embed_dim * self.output_num_cls, out_no_seq=True
        )
    ...
    def _aggregate_embeddings(self, embeddings: Tensor, key_mask: Optional[Tensor] = None) -> Tensor:
        ...
        if self.output_num_cls != self.num_cls:
            group_size = self.num_cls // self.output_num_cls
            cls_outputs = cls_outputs.reshape(
                *cls_outputs.shape[:-2], self.output_num_cls, group_size, self.embed_dim
            ).mean(dim=-2)

        return cls_outputs.flatten(-2)  # (B, T, C*E)

    def refine_full_tokens(self, embeddings: Tensor, d: Optional[Tensor] = None) -> Tensor:
        """Run the shared row stack while retaining every token.

        CR2 uses this only for its first row pass.  It deliberately reuses
        ``tf_row`` and ``cls_tokens`` rather than constructing another row
        module.  The input is cloned so the ordinary E4 path keeps its
        historical in-place CLS insertion semantics unchanged.
        """

        B, T, HC, _ = embeddings.shape
        device = embeddings.device
        full_tokens = embeddings.clone()
        full_tokens[:, :, : self.num_cls] = self.cls_tokens.expand(
            B, T, self.num_cls, self.embed_dim
        ).to(device)

        if d is None:
            key_mask = None
        else:
            valid_tokens = d + self.num_cls
            indices = torch.arange(HC, device=device).view(1, 1, HC).expand(B, T, HC)
            key_mask = indices >= valid_tokens.view(B, 1, 1)

        rope = self.tf_row.rope
        if self.recompute:
            for block in self.tf_row.blocks:
                full_tokens = checkpoint(
            ...
            for block in self.tf_row.blocks:
                full_tokens = block(full_tokens, key_padding_mask=key_mask, rope=rope)

        full_tokens = self.out_ln(full_tokens)
        if key_mask is not None:
            full_tokens = full_tokens.masked_fill(key_mask.unsqueeze(-1), 0.0)
        return full_tokens
