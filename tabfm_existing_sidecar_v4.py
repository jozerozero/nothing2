#!/usr/bin/env python3
"""V4: lower-memory operational lane for manifest-proven small full-data tasks.
...
GIB = 1024**3
SMALL_TASK_LIMITS = {"max_total_rows": 2048, "max_features": 100, "max_classes": 10,
...
def load_plan(path, fresh=False):
    ...
    require(p["max_lanes"] == 1 and p["cpu_count"] == 4 and p["mem_gib"] == 20
            ...
            "Requires one4CPU/20GiB child inside verified64GiB/eight-GPU parent")
    require(p["free_cpu_cores"] >= 4 and 0 <= p["startup_other_rss_bytes"] <= 40 * GIB, "Insufficient CPU/RSS headroom")
    require(p.get("small_task_limits") == SMALL_TASK_LIMITS, "Small-task dispatch protocol mismatch")
    require("regression_shape_overlay" in p, "V4 requires its pinned regression-shape overlay")
    now = time.time()
    ...
    shape_helper = Path(__file__).with_name("tabfm_regression_shapes.py").resolve()
    require(sum(Path(source["path"]).resolve() == shape_helper for source in p.get("source_records", [])) == 1,
            "V4 plan must uniquely pin its regression shape helper")
...
def guard_snapshot(snapshot, startup=False):
    reasons = []
    if snapshot["own_tree_rss_bytes"] > 16 * GIB:
        reasons.append("own_process_tree_exceeds16GiB")
    if snapshot["same_uid_rss_bytes"] > 60 * GIB:
        reasons.append("same_uid_node_RSS_exceeds60GiB")
    if startup and snapshot["other_same_uid_rss_bytes"] > 40 * GIB:
        reasons.append("startup_other_RSS_exceeds40GiB")
    if reasons:
        raise ResourceGuardError("resource_guard: " + ",".join(reasons))


def bind_idle_cpu_cores():
    """Inherit64 CPU access, then bind this lane and all fresh workers to four."""
    import psutil
    require(hasattr(os, "sched_getaffinity") and hasattr(os, "sched_setaffinity"),
            "Linux CPU affinity APIs are mandatory")
    allowed = set(os.sched_getaffinity(0))
    require(len(allowed) >= 64, "Child must inherit the parent64 CPU access before selection")
    frames = [psutil.cpu_percent(interval=.5, percpu=True) for _ in range(2)]
    require(all(all(index < len(frame) and isinstance(frame[index], (int, float))
                    and math.isfinite(frame[index]) and 0 <= frame[index] <= 100
                    for index in allowed) for frame in frames),
            "CPU utilization observations incomplete or invalid")
    candidates = [index for index in allowed if all(frame[index] < 25 for frame in frames)]
    require(len(candidates) >= 4, "Fewer than four CPUs idle in both utilization observations")
    selected = sorted(candidates, key=lambda index:
                      (max(frame[index] for frame in frames),
                       sum(frame[index] for frame in frames), index))[:4]
    os.sched_setaffinity(0, set(selected))
    actual = set(os.sched_getaffinity(0))
    require(actual == set(selected), "CPU affinity mask differs from the four selected CPUs")
    return {"inherited_cpu_access_count": 64, "observed_allowed_cpu_count": len(allowed),
            "allowed_cpu_ids_before": sorted(allowed), "actual_model_cpu_count": 4,
            "selected_cpu_ids": sorted(selected), "actual_cpu_ids_after": sorted(actual),
            "sample_window_seconds": .5, "sample_count": 2, "idle_threshold_percent_exclusive": 25,
            "utilization_samples": [{str(i): frame[i] for i in sorted(allowed)} for frame in frames]}
...
def task_eligibility(task, shape_overlay=None):
    """Eligibility only; never slice, resplit, encode, or alter model data."""
    row = task["row"]
    shape = row
    shape_source = "frozen_manifest_row"
    overlay_record = None
    if task["task_kind"] == "regression" and shape_overlay:
        overlay_record = shape_overlay.get(str(task["dataset_index"]))
        if overlay_record is not None:
            shape = overlay_record
            shape_source = "verified_pinned_original_step22175_receipt"
    ...
    if not known:
        reasons.append("missing_or_invalid_exact_manifest_dimensions")
    else:
        if dimensions["train_rows"] + dimensions["test_rows"] > SMALL_TASK_LIMITS["max_total_rows"]:
            reasons.append("total_rows_exceed2048")
        if dimensions["features"] > SMALL_TASK_LIMITS["max_features"]:
            reasons.append("features_exceed100")
    if task["task_kind"] == "classification":
        classes = row.get("classes")
        if not isinstance(classes, int) or isinstance(classes, bool) or classes < 2:
            reasons.append("missing_or_invalid_class_count")
        elif classes > SMALL_TASK_LIMITS["max_classes"]:
            reasons.append("classification_classes_exceed10")
    elif task["task_kind"] != "regression":
        reasons.append("unknown_task_kind")
    return {"task_kind": task["task_kind"], "dataset_index": task["dataset_index"],
            "dataset": task["dataset"], "suite": row.get("suite"), "eligible": not reasons,
            "exact_eligibility_dimensions": dimensions, "classes": row.get("classes"),
            "input_fingerprint": row.get("input_fingerprint"), "reasons": reasons,
            "shape_source": shape_source, "shape_receipt": overlay_record.get("receipt") if overlay_record else None}


def eligibility_audit(plan, man, tasks, shape_overlay=None):
    records = [task_eligibility(task, shape_overlay) for task in tasks]
    counts = {kind: sum(r["eligible"] and r["task_kind"] == kind for r in records)
              for kind in ("classification", "regression")}
    return {"manifest_id": man["manifest_id"], "plan_id": plan["plan_id"],
            "small_task_limits": SMALL_TASK_LIMITS, "eligible_counts": counts,
            "eligible_total": sum(counts.values()), "records": records,
            "unknown_shapes_left_unclaimed": True, "large_and_hierarchical_formal_tasks_left_unclaimed": True,
            "full_original_support_and_test_unchanged": True,
            "smoke_exempt_from_size_filter_but_same16GiB_operational_guard": True}
...
def lane(plan, man, tasks):
    ...
    tmpdir_audit = activate(man, plan, root / "runtime_environment")
    from tabfm_regression_shapes import validate
    shape_overlay = validate(plan["regression_shape_overlay"], man, tasks)
    eligible = eligibility_audit(plan, man, tasks, shape_overlay)
    frozen.atomic(man, root / "eligibility.json", eligible)
    parent, memory = query_parent(plan), rss_snapshot()
    guard_snapshot(memory, startup=True)
    cpu_binding = bind_idle_cpu_cores()
    ...
    frozen.atomic(man, root / "preflight.json", {"owner": owner, "actual_gpu": actual, "parent": parent,
        ...
        "cpu_binding": cpu_binding, "tmpdir_runtime_audit": tmpdir_audit,
        "parent_gpu_reservation_count": 8, "step_inherited_gpu_access_count": 8, "actual_model_gpu_count": 1,
        "operational_guards": {"child_mem_gib": 20, "own_tree_gib": 16, "same_uid_gib": 60,
                               "startup_other_gib": 40, "nice": os.getpriority(os.PRIO_PROCESS, 0)}})
    ...
    selected = frozen.smoke_tasks(man, tasks)  # All four ORIGINAL full-data smokes; no size filtering.
    ...
    try:
        ...
        eligible_tasks = [task for task in tasks if task_eligibility(task, shape_overlay)["eligible"]]
        for task in sorted(eligible_tasks, key=lambda t: (frozen.work_size(t), t["task_kind"], t["dataset_index"])):
        ...
        summary["reason"] = summary["reason"] or "no unclaimed eligible small work; not campaign completion"
...
def step_command(plan, man, plan_path):
    ...
    # Inherit parent64 CPU access, then bind the lane to four measured idle CPUs.
    ...
    return ["srun", "--jobid=" + PARENT, "--overlap", "--exact", "--nodes=1", "--ntasks=1",
            "--cpus-per-task=64", "--mem=20G", "--gpus=8", "--gpu-bind=none", "--cpu-bind=none",
